...
# It's possible to edit sensor channels as well
print(wrapped_packet.microphone_channel().sample_rate_hz())
wrapped_packet.microphone_channel().set_sample_rate_hz(800.0)
print(wrapped_packet.microphone_channel().sample_rate_hz())

# We can also assign a sensor channel to a variable and edit it.
barometer_sensor = wrapped_packet.barometer_channel()
barometer_sensor.set_sensor_name("really_awesome_barometer")
print(wrapped_packet.barometer_channel().sensor_name())

# Or, we can create a brand new channel and set it on the packet
...
wrapped_packet.set_light_channel(light_sensor)
print(wrapped_packet.light_channel())

# Finally, we can also remove a sensor channel from a packet by passing "None" to the set method.
wrapped_packet.set_gyroscope_channel(None)
print(wrapped_packet.has_gyroscope_channel())

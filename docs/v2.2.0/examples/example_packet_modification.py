...
# We can also remove a sensor sensor from a packet by passing "None" to the set method.
wrapped_packet.set_gyroscope_sensor(None)
print(wrapped_packet.has_gyroscope_sensor())

# We can update timestamps in non-microphone and non-time synchronization sensors.
# Let's find a timestamp.
print(wrapped_packet.barometer_sensor().timestamps_microseconds_utc()[0])
# The time is in microseconds, so this example moves the clock forward by 1 second
wrapped_packet.update_uneven_sensor_timestamps(1000000)
print(wrapped_packet.barometer_sensor().timestamps_microseconds_utc()[0])
# Use negative values to adjust time backwards.
wrapped_packet.update_uneven_sensor_timestamps(-1000000)
print(wrapped_packet.barometer_sensor().timestamps_microseconds_utc()[0])

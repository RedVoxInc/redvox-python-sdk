...
class SensorDataPa:
    ...
    def __init__(
        ...
        :param sensor_data: pyarrow table with the timestamps and sensor data; first column is always the timestamps,
    ...
    def from_dict(
        ...
        return SensorDataPa(sensor_name, pa.Table.from_pydict(sensor_data), sensor_type, sample_rate_hz,
                            ...
                            calculate_stats, use_offset_model_for_correction)
    ...
    def organize_and_update_stats(self) -> "SensorDataPa":
        """
    ...
    def get_data_channel(self, channel_name: str) -> Union[np.array, List[str]]:
        ...
        """
        if channel_name not in self._arrow.schema.names:
            raise ValueError(
                f"WARNING: {channel_name} does not exist; try one of {self._arrow.schema.names}"
            )
        if channel_name == "location_provider":
            return [LocationProvider(c).name for c in self._arrow[channel_name]]
        elif channel_name == "image_codec":
            return [ImageCodec(c).name for c in self._arrow[channel_name]]
        elif channel_name == "audio_codec":
            return [AudioCodec(c).name for c in self._arrow[channel_name]]
        elif channel_name == "network_type":
            return [NetworkType(c).name for c in self._arrow[channel_name]]
        elif channel_name == "power_state":
            return [PowerState(c).name for c in self._arrow[channel_name]]
        elif channel_name == "cell_service":
            return [CellServiceState(c).name for c in self._arrow[channel_name]]
        return self._arrow[channel_name].to_numpy()
    ...
    def data_channels(self) -> List[str]:
        ...
        """
        return self._arrow.schema.names
        ...
        if self.type == SensorType.AUDIO:
            ...
            timestamps = pa.array(
                calc_evenly_sampled_timestamps(
                    ...
                    self.num_samples(),
                    slope))
            self._arrow = self._arrow.set_column(0, "timestamps", timestamps)
        else:
            self._arrow = self._arrow.set_column(0, "timestamps",
    ...
    def sort_by_data_timestamps(self, ascending: bool = True):
        ...
        if ascending:
            ...
            order = "descending"
        indic = pc.sort_indices(self._arrow, sort_keys=[("timestamps", order)])
        self._arrow = pc.take(self._arrow, indic)
    ...
    def interpolate(self, interpolate_timestamp: float, first_point: int, second_point: int = 0,
        ...
        """
        start_point = self._arrow.slice(first_point, 1).to_pydict()
        if not copy and second_point:
            i_p = {}
            end_point = self._arrow.slice(first_point + second_point, 1).to_pydict()
            first_closer = \
                np.abs(start_point[0] - interpolate_timestamp) \
                <= np.abs(end_point[0] - interpolate_timestamp)
            for col in self._arrow.schema.names:
        ...
        # d = pd.DataFrame(pd.concat([numeric_diff, non_numeric_diff]))

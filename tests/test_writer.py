...
class WriteAndSetModules(ArraysTestCase):
    def test_write(self):
        ...
        mic_sen = rar.MicrophoneSensor(wrapped_packet._get_channel(api900_pb2.MICROPHONE))
        time_sen = rar.TimeSynchronizationSensor(wrapped_packet._get_channel(api900_pb2.TIME_SYNCHRONIZATION))
        mag_sen = rar.MagnetometerSensor(wrapped_packet._unevenly_sampled_channels[1])
        gyro_sen = rar.GyroscopeSensor(wrapped_packet._unevenly_sampled_channels[-5])
        light_sen = rar.LightSensor(wrapped_packet._get_channel(api900_pb2.LIGHT))
        bar_sen = rar.BarometerSensor(wrapped_packet._get_channel(api900_pb2.BAROMETER))
        accel_sen = rar.AccelerometerSensor(wrapped_packet._unevenly_sampled_channels[-2])
        loc_sen = rar.LocationSensor(wrapped_packet._unevenly_sampled_channels[6])
        ...
        mic_chan.__set_sensor_name("Test Microphone")
        mic_chan.__set_sample_rate_hz(42.0)
        mic_chan.__set_first_sample_timestamp_epoch_microseconds_utc(1)
        ...
        mic_chan.__set_payload(wrapped_packet._evenly_sampled_channels[0].__payload, 1,
                               wrapped_packet._evenly_sampled_channels[0].__get_payload_type())
        ...
        mic_chan.__set_channel(wrapped_packet._evenly_sampled_channels[0].__protobuf_channel)
        mic_chan.__set_metadata(["This", "is a Test Sensor Packet", "Don't", "Take this data too seriously"])
        mic_chan.__update_stats()

        # Access functions.  Asserts used to ensure values set correctly:
        self.assertSampledArray(mic_chan.__get_payload(api900_pb2.MICROPHONE),
                                4096,
                                [0, 2048, 4095],
                                [201, -4666, -1867])
        self.assertEqual("int32_payload", mic_chan.__get_payload_type())
        self.assertSampledArray(mic_chan.__get_multi_payload([api900_pb2.MICROPHONE]),
                                4096,
                                [0, 2048, 4095],
                                [201, -4666, -1867])
        self.assertAlmostEqual(mic_chan.__get_value_mean(api900_pb2.MICROPHONE), -127.91796875, 1)
        self.assertAlmostEqual(mic_chan.__get_value_std(api900_pb2.MICROPHONE), 2455.820326625975, 3)
        self.assertAlmostEqual(mic_chan.__get_value_median(api900_pb2.MICROPHONE), -123.0, 1)
        self.assertTrue("MICROPHONE" in mic_chan.__get_channel_type_names())
        self.assertEqual(mic_chan.__sensor_name, "I/INTERNAL MIC")
        self.assertAlmostEqual(mic_chan.__sample_rate_hz, 80.0, 1)
        self.assertEqual(mic_chan.__first_sample_timestamp_epoch_microseconds_utc, 1532656864354000)
        self.assertTrue(api900_pb2.MICROPHONE in mic_chan.__channel_types)
        self.assertAlmostEqual(mic_chan.__value_means[0], -127.91796875, 1)
        self.assertAlmostEqual(mic_chan.__value_stds[0], 2455.820326625975, 3)
        self.assertAlmostEqual(mic_chan.__value_medians[0], -123.0, 1)
        self.assertEqual(mic_chan.__channel_index(api900_pb2.MICROPHONE), 0)
        self.assertTrue(mic_chan.__has_channel(api900_pb2.MICROPHONE))
        self.assertFalse(mic_chan.__has_channel(api900_pb2.BAROMETER))
        self.assertEqual(len(mic_chan.__metadata), 4)
        self.assertListEqual(mic_chan.__metadata,
                             ["This", "is a Test Sensor Packet", "Don't", "Take this data too seriously"])
        synthetic_dict = mic_chan.__metadata_as_dict()
        ...
        self.assertTrue(mic_chan.__payload is not None)
        self.assertTrue(mic_chan.__protobuf_channel is not None)
        ...
        mic_chan = rar.EvenlySampledChannel().even_create(
            ...
            [api900_pb2.MICROPHONE], "int32_payload", wrapped_packet._evenly_sampled_channels[0].__payload, 1, 80.0,
            1532656864354000)
        ...
        newpacket._add_channel(mic_sen.__evenly_sampled_channel)
        newpacket._add_channel(time_sen._unevenly_sampled_channel)
        newpacket._add_channel(mag_sen.__unevenly_sampled_channel)
        newpacket._add_channel(gyro_sen.__unevenly_sampled_channel)
        newpacket._add_channel(light_sen.__unevenly_sampled_channel)
        newpacket._add_channel(bar_sen.__unevenly_sampled_channel)
        newpacket._add_channel(accel_sen.__unevenly_sampled_channel)
